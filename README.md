...
│   │   ├── users.json.example    # Пример структуры, скопируйте в users.json
│   │   └── users.json            # Рабочий файл, игнорируется Git
...
После установки зависимостей создайте файл базы пользователей:
```bash
cp backend/db/users.json.example backend/db/users.json
```
...
При старте backend:
- Express-сервер поднимается на :3001
- Все изображения сохраняются в `/uploads` и доступны по `/uploads/:filename`
- Файл `backend/db/users.json` (скопируйте из `users.json.example`) используется как "база данных"
...
│   │   ├── users.json.example # Пример, копировать в users.json
│   │   └── users.json        # Рабочий файл (в .gitignore)
...
- Backend запускается на порту 3001, хранит данные в `backend/db/users.json` (создайте, скопировав `users.json.example`).
