...
- Перед первым запуском убедитесь, что файл `backend/db/users.json` существует. Если его нет, создайте пустой файл со строкой `[]` внутри, иначе сервер завершится с ошибкой.
